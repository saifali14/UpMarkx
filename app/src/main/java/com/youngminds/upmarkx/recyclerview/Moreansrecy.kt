...
class Moreansrecy(val context: Context, val answer: Answer,val question:Question):Item<ViewHolder>() {
    ...
    override fun bind(viewHolder: ViewHolder, position: Int) {
            ...
            currentuser?.let { it1 ->
                dblike.set(it1).addOnSuccessListener {
                    ...
                    val db = firestore.collection("UserInfo").document(answer.userid.toString())
                        .collection("Likes").document(randomid)
        ...
        viewHolder.itemView.moreanscard_comment.setOnClickListener {
            ...
            Glide.with(context).load(currentuser?.imageuri).into(view.moreans_comment_imageview)
